...
const AddToCard = () => {
  ...
  useEffect(() => {
    ...
    if (offerElement && offerElement.getAttribute('href')) {
      setOfferLink(offerElement.getAttribute('href') || '');
    }
  }, []);
        ...
        {cartItems.length > 0 && (
          <div className="px-5 pb-5 flex flex-col gap-5 h-full">
            ...
            <div className="border-t border-muted mt-auto pb-5">
              ...
              <Button className="w-full text-lg text-white" size="lg" asChild>
                <a href={offerLink} className={`${totalItems > 1 && 'pointer-events-none opacity-50'}`}>
                  Buy Now
                </a>
              </Button>
            </div>
